...
var urlroot = 'https://delinea.com/blog';
var prdroot = '';
var opnroot = '';
var sourceb = '';   // category - cti prd opn

const UNIQDAT = true;  // create image folder for each blog or not
const TITLEST = "MMM";
const REGULAR = ".blog-index__post.blog-index__post--small";
const FEATURE = "MMM";
const PAGESTY = '';     // page style

var IDXTITL = ".blog-index__post-title";
var IDXAUTH = ".MMM";

var IDXDATE = ".MMM";
var BDYDATE = ".post-MMM-date";

var BDYBLOG = ".blog-post";
var DBYAUTH = '.MMM';
var BDYTAGS = ".blog-post__tag-link";
...
function parse_blog_url($, section){
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        //if(i==0) { console.log(item_json); process.exit(1); }
...
async function save_blog_content(url, file){   
    ...
    // get date from "datePublished": "2022-05-03 12:00:00",
    let jsdate, bdate;
    let reg = data.match(/datePublished": "(\d\d\d\d-\d\d-\d\d) /);
    if(reg.length > 0) jsdate = reg[1];
    ...
    if( bodym.match('article.article--single') ){
        btags = '';  // tags in attr;  doubleoctopus
        let cats = $('.article.article--single').attr('class').split(' ');
    ...
    bdate = jsdate.match(/\w+/) ? jsdate : find_post_date($, datas, dataa);
    ...
    if(urlroot.match('appgate') ||
       ...
       urlroot.match('doubleoctopus') ||
       urlroot.match('gurucul')
       )
...
`;
    ...
    for(let i=0; i < gg.length; i++ ){       
        ...
        if (! UNIQDAT)   filelocal = datroot + '/' + imagename;
        ...
        var sss = imgsrc, imglnk = '../data/' + uniq + '/' + imagename;
        if( imgsrc.includes("?")) sss = imgsrc.split('?')[0];
        if(! UNIQDAT) imglnk = '../data/' + imagename;
...
switch (arg[0]) {
    case '--save-urls':
        ...
        get_all_blog_url2array(urlroot, false, 20);
        break;
    case '--check-new':
