...
var urlroot = 'https://www.dragos.com/blog';
var prdroot = '';
var opnroot = '';
...
const TITLEST = "";
const REGULAR = ".SearchResultsModule-results-item";
const FEATURE = ".MMM";
const PAGESTY = ''; // page style

var IDXTITL = ".PagePromo-title";
var IDXAUTH = ".MMM";
var IDXLINK = "a";

var IDXDATE = ".PagePromo-date";
var BDYDATE = ".Page-datePublished";

var BDYBLOG = ".Page-main";
var DBYAUTH = '.Page-authors';
var BDYTITL = ".Page-headline";
var DATAUTH = ".AssetTitle-footer";
var BDYTAGS = ".MMMM";
var IMGSTYL = ""; //width: 100%

var RMPARA0 = new RegExp('today to learn how your organization can', 'i');
...
let rootfld = bserver;
if(urlroot.match(/labs/)) rootfld = 'labs.' + bserver;
...
function parse_blog_url($, section){
	...
	console.log("blogs found:", posts.length);
...
async function find_blog_links(purl){
    ...
    $(".SearchResultsModule-aside").remove();
...
async function save_blog_content(url, file){   
    ...
    //if(url.match('blog/webinar-')) return [null, null, null]; //event
    ...
    if(data.length < 100) {
		console.log("error axio %s".red, url);
    ...
    let urihp = www[1],  bodyt = BDYBLOG, 
        dateI = IDXDATE, dateB = BDYDATE, tagaa = BDYTAGS;
    ...
    bdate = jsdate != null && jsdate.match(/\w+/) ? jsdate : find_post_date($, dateI, dateB);
    ...
    // no late load
    bodym = bodym.replaceAll('<div class="Page-articleBody">', '<br><div class="Page-articleBody">');
...
`;
    ...
    for(let i=0; i < gg.length; i++ ){       
		...
		//https://dragos.brightspotcdn.com/dims4/default/64eaf8f/2147483647/strip/true/crop/1800x1165+0+0/resize/1440x932!/format/webp/quality/90/?url=https%3A%2F%2Fdragos-production-dragos.s3.us-west-2.amazonaws.cFbrightspot%2Ff6%2F63%2Fa1fa897e427ca9b86dabeb0d8c78%2Fransomware-map-q3-2025.jpg

		let reg = imgurl.match(/http[s]:\/\/.*\?url=(http.+)/);
		if( reg != null){
			imgurl = reg[1].replace("%3A", ':');
...
async function get_all_blog_url2array(urlp, firstpage_only=false, finalpage=0) {
    ...
    var loadhtml;
    // onepage html is saved manually for 1st parsing
	if ( fs.existsSync(onepage) ){
        ONEPAGE = true;
    ...
    for(var i = 0, j=1; i < blogs.length; i++) {
        ...
        try {
            ...
            if(bfilelist.match(f) ){    // blog file exists
                ...
                if(obj.bfile == null){
                    console.log('error:'.red, obj.blink);
                    continue;
                }
...
function remove_html_paragraph($){   
    let pp = $('p');
    
    for(let i = 0; i< pp.length; i++){
        if($(pp[i]).html().match(RMPARA0)){
            process.stdout.write(`P`);
            $(pp[i]).remove(); //cybereason
        }
    }
}

function remove_html_sections($){
	$(".TableOfContent.mobile").remove();
	$(".Page-articleBelow").remove();
	$(".Page-readTime").remove();
    ...
    $(".no-border").remove(); // author
    remove_html_paragraph($);
}
