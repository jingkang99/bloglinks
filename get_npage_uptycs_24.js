...
var urlroot = 'https://www.uptycs.com/blog/';
var prdroot = '';
var opnroot = '';
...
var IDXDATE = ".read-time .date";
var BDYDATE = "MMM";

var BDYBLOG = ".body .content";
var DBYAUTH = '.MMM';
var BDYTAGS = ".topics .link";
var BDYTITL = ".hs_cos_wrapper_type_text";
var IMGSTYL = "width: 100%"; //width: 100%
...
function parse_blog_url($, section){
    ...
    //console.log(posts.length); console.log( $(posts[0]).html() ); process.exit(1); 
    for (let i = 0; i < posts.length; i++) {
		...
		//only 'section' has link - add parent()
        let postLinkWrapper = $(posts[i]).parent().find(IDXLINK)[i],
...
async function save_blog_content(url, file){   
    ...
    for (let i = 0, j = 0; i < tagbb.length && j < 3; i++) {
        ...
        if( $(tagbb[i]).text().match(/(Featured|Company|News|Culture|Announcement)/)) continue;
	...
	let lh2 = $('h2'); 
	if( lh2.length > 1){
		if( $( lh2[lh2.length - 1] ).text().match(/(To learn more about|To learn about)/i) ) 
			$( lh2[lh2.length - 1] ).remove();
		if( $( lh2[lh2.length - 2] ).text().match(/(To learn more about|To learn about)/i) ) 
			$( lh2[lh2.length - 2] ).remove();
	}
...
switch (arg[0]) {
    case '--save-urls':
        ...
        get_all_blog_url2array(urlroot, false, 21);
        break;
    case '--check-new':
