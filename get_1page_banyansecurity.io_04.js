...
const urlroot = 'https://www.banyansecurity.io/blog/';
const REGULAR = ".type-post";
const FEATURE = ".NONONO";
const BODYBLG = "#left-area";
...
function parse_blog_url($, section){
    ...
    for (let i = 0; i < posts.length; i++) {
        let postTitleWrapper = $(posts[i]).find(".entry-title")[0],
        ...
        let authorWrapper = $(posts[i]).find(".author")[0];
        if(authorWrapper != null ) author = $(authorWrapper).text().replace('By  ', '').replace(' |', '');
        author = author.trim();

        let dateWrapper = $(posts[i]).find(".published")[0];
        ...
        if(! postDate.match(',')){  // format to MM DD, YYYY
            let datetemp = postDate.split(" ");
...
<body class="post-template-default single single-post single-format-standard"><br><div class="container">
`;
        ...
        if(firstpage_only){ // only check 1st page on new blog
            ...
            pburls.forEach( e => {
                ...
                if(! bfilelist.match(f)){
                    ...
                    console.log("  %s %s".green, action, f.slice(0, 50) );
                }else{
