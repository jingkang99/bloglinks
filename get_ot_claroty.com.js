...
const urlroot = 'https://claroty.com/blog';
const REGULAR = "a.entry-card.block";
const FEATURE = ".MMM";
const BODYBLG = ".rich-text";

const BDYTITL = "h1.h-6xl";
const DATAUTH = ".flex-wrap.items-center";
...
function parse_blog_url($, section){
    ...
    for (let i = 0; i < posts.length; i++) {
        // blog title
		let postTitleWrapper = $(posts[i]).find(".h-2xl.!mt-2")[0],
        ...
        let author = 'Claroty', postDate = 'January 5, 2000', postDesc=title;
...
async function find_blog_links(purl){
	// https://claroty.com/blog?blog_posts=1

    let num, mat = purl.match(/http[s]:\/\/.+=(\d+)/);
...
async function save_blog_content(url, file){   
    ...
    // <span itemprop="datePublished" content="2025-09-05T15:33:00+00:00">September 5th, 2025</span>
    let jsdate, bdate;
    let reg = data.match(/datePublished" content="(\d\d\d\d-\d\d-\d\d)T/);
    if( reg != null && reg.length > 0) jsdate = reg[1];

    var $ = cheerio.load(data);
	$("h3 a").remove();
	$(".relative.py-4").remove();
	$(".mx-2.text-subtle").remove();
	$(".relative.text-light").remove();
...
switch (arg[0]) {
    case '--save-urls':
        ...
        get_all_blog_url2array(urlroot, false, 27);
        break;
    case '--check-new':
