...
const urlroot = 'https://www.radiflow.com/category/blog/';
const REGULAR = ".row.resorces-wrapper a.resorce";
const FEATURE = ".MMM";

const IDXTITL = ".resorce-content p";
const IDXAUTH = ".MMM";
const IDXLINK = "a";
...
function parse_blog_url($, section){
    ...
    for (let i = 0; i < posts.length; i++) {
        ...
        let author = 'radiflow', postDate = 'January 5, 2000', postDesc=title;
        ...
        let dateWrapper = postDate = $(posts[i]).find(IDXDATE)[0];
...
function find_blog_links(purl){
	// https://www.radiflow.com/category/blog/page/3/
...
function get_all_blog_url2array(urlp, firstpage_only=false, finalpage=0) {
	...
	let stopN = Date.now();
    let stopt = process.hrtime(stime);
    console.log(`\nblog array: ${stopN - stimN} milliseconds`.yellow);
...
switch (arg[0]) {
    case '--save-urls':
        ...
        get_all_blog_url2array(urlroot, false, 18);
        break;
    case '--check-new':
